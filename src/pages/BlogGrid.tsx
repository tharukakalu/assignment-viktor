...
const BlogGrid: React.FC = () => {
  ...
  const [categoryId, setCategoryId] = useState<number | null>(initialState.categoryId);
  const [authorId, setAuthorId] = useState<number | null>(initialState.authorId);
  const [sortOrder, setSortOrder] = useState<'DESC' | 'ASC'>(initialState.sortOrder);
  ...
  useEffect(() => {
    ...
    loadBlogPosts();
  }, [currentPage, debouncedSearch, categoryId, authorId, sortOrder, pageSize]);
      ...
      <header className="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-sm text-blue-100 font-medium">Engineering Innovation Hub</span>
              </div>
            </div>
          </div>
          
          <p className="text-lg text-blue-50 max-w-2xl leading-relaxed">
            Discover insights on parametric design, digital transformation, and cutting-edge engineering solutions
          </p>
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <svg className="w-full h-12" preserveAspectRatio="none" viewBox="0 0 1200 120" xmlns="http://www.w3.org/2000/svg">
            <path d="M321.39,56.44c58-10.79,114.16-30.13,172-41.86,82.39-16.72,168.19-17.73,250.45-.39C823.78,31,906.67,72,985.66,92.83c70.05,18.48,146.53,26.09,214.34,3V0H0V27.35A600.21,600.21,0,0,0,321.39,56.44Z" fill="#f8fafc" />
          </svg>
        </div>
      </header>
